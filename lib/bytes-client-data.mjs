...
function requireBlockHashOrNull(value, path) {
  if (value !== null && (typeof value !== 'string' || !/^0x[0-9a-f]{64}$/.test(value))) fail(path, 'must be a canonical block hash or null');
}
...
export function validateBytesMetricsResponse(value) {
  ...
  const avalanche = requireObject(provenance.avalanche, 'metrics.provenance.avalanche');
  requireNonEmptyString(avalanche.chain, 'metrics.provenance.avalanche.chain');
  if (avalanche.chainId !== 43_114) fail('metrics.provenance.avalanche.chainId', 'must equal Avalanche C-Chain ID 43114');
  requireSourceBlock(avalanche.sourceBlock, 'metrics.provenance.avalanche.sourceBlock');
  requireBlockHashOrNull(avalanche.sourceBlockHash, 'metrics.provenance.avalanche.sourceBlockHash');
  if (avalanche.asOf !== null) requireCanonicalTimestamp(avalanche.asOf, 'metrics.provenance.avalanche.asOf');
  for (const name of ['bytesTokenContract', 'proxyImplementation', 'ccipBurnMintPool', 'tokenIdentityVerification']) requireNonEmptyString(avalanche[name], `metrics.provenance.avalanche.${name}`);
  if (typeof avalanche.tokenIdentityVerified !== 'boolean') fail('metrics.provenance.avalanche.tokenIdentityVerified', 'must be boolean');
  requireNonEmptyString(provenance.crossChainSupplyTreatment, 'metrics.provenance.crossChainSupplyTreatment');
  const priceSource = requireObject(provenance.priceSource, 'metrics.provenance.priceSource');
  if (typeof priceSource.verified !== 'boolean') fail('metrics.provenance.priceSource.verified', 'must be boolean');
  for (const name of ['verification', 'dextoolsPairUrl', 'uniswapV3Pool', 'uniswapV3Factory', 'quoteToken', 'ethUsdFeed', 'quoteCurrency', 'method']) requireNonEmptyString(priceSource[name], `metrics.provenance.priceSource.${name}`);
  requirePositiveInteger(priceSource.feeTier, 'metrics.provenance.priceSource.feeTier');
  ...
  const pendingSnapshot = requireObject(provenance.pendingRewardsSnapshot, 'metrics.provenance.pendingRewardsSnapshot');
  requireSourceBlock(pendingSnapshot.sourceBlock, 'metrics.provenance.pendingRewardsSnapshot.sourceBlock');
  requireBlockHashOrNull(pendingSnapshot.sourceBlockHash, 'metrics.provenance.pendingRewardsSnapshot.sourceBlockHash');
  if (pendingSnapshot.asOf !== null) requireCanonicalTimestamp(pendingSnapshot.asOf, 'metrics.provenance.pendingRewardsSnapshot.asOf');
  ...
  for (const name of ['freshForSeconds', 'pendingRewardsSeconds', 'participantSnapshotSeconds', 'staleWhileRevalidateSeconds', 'staleIfErrorSeconds']) {
  ...
  validateScalarMetric(metrics.projectedNext365DayIssuance, 'metrics.metrics.projectedNext365DayIssuance', { nonnegative: true });
  if (metrics.projectedNext365DayIssuance.classification !== 'projected') fail('metrics.metrics.projectedNext365DayIssuance.classification', 'must be projected');
  ...
  validateScalarMetric(metrics.avalancheBytesSupply, 'metrics.metrics.avalancheBytesSupply', { nonnegative: true });
  ...
  for (const name of [
    ...
    's2CollectionSupply',
    's2StakedPercentage',
    'ethereumBytesHolderCount',
    'avalancheBytesHolderCount',
    'crossChainUniqueBytesHolderCount',
  ]) validateScalarMetric(metrics[name], `metrics.metrics.${name}`, { nonnegative: true });
  validateScalarMetric(metrics.pendingUnclaimedRewards, 'metrics.metrics.pendingUnclaimedRewards', { nonnegative: true });
  validateScalarMetric(metrics.bytesPriceUsd, 'metrics.metrics.bytesPriceUsd', { nonnegative: true });
  validateScalarMetric(metrics.totalSupplyValuationUsd, 'metrics.metrics.totalSupplyValuationUsd', { nonnegative: true });
  validateScalarMetric(metrics.circulatingMarketCapUsd, 'metrics.metrics.circulatingMarketCapUsd', { nonnegative: true });
  ...
  const pendingAvailable = metrics.pendingUnclaimedRewards.availability === 'available';
  if (pendingAvailable !== (pendingSnapshot.sourceBlock !== null && pendingSnapshot.sourceBlockHash !== null && pendingSnapshot.asOf !== null)) fail('metrics.provenance.pendingRewardsSnapshot', 'must match pending-reward availability');
  if (pendingAvailable && metrics.pendingUnclaimedRewards.asOf !== pendingSnapshot.asOf) fail('metrics.metrics.pendingUnclaimedRewards.asOf', 'must match pending snapshot provenance');
  const avalancheAvailable = metrics.avalancheBytesSupply.availability === 'available';
  if (avalanche.tokenIdentityVerified !== avalancheAvailable) fail('metrics.provenance.avalanche.tokenIdentityVerified', 'must match Avalanche supply availability');
  if (avalanche.tokenIdentityVerified !== (avalanche.sourceBlock !== null && avalanche.sourceBlockHash !== null && avalanche.asOf !== null)) fail('metrics.provenance.avalanche', 'identity and source evidence must be coherent');
  if (metrics.bytesPriceUsd.availability !== metrics.totalSupplyValuationUsd.availability) fail('metrics.metrics.totalSupplyValuationUsd', 'must match BYTES price availability');
  if (priceSource.verified !== (metrics.bytesPriceUsd.availability === 'available')) fail('metrics.provenance.priceSource.verified', 'must match price and valuation availability');
  if (metrics.totalSupplyValuationUsd.availability === 'available' && metrics.ethBytes2Supply.availability !== 'available') fail('metrics.metrics.totalSupplyValuationUsd', 'requires canonical Ethereum supply');
  if (metrics.circulatingMarketCapUsd.availability !== 'unavailable') fail('metrics.metrics.circulatingMarketCapUsd', 'must remain unavailable until circulating supply is established');
