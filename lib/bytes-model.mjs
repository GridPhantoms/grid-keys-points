...
export function projectedIssuanceOverDays(daily, fractionOfCurrentWeekElapsed = 0, days = 365) {
  ...
  if (!Number.isFinite(days) || days <= 0) throw new RangeError('days must be a finite positive number');

  const weeklyDecayRatio = 2 ** (-1 / DECAY_WEEKS_PER_HALF);
  let remainingDays = days;
  let dailyRate = daily;
  let total = 0;
  const remainingCurrentWeek = Math.min(remainingDays, DAYS_PER_WEEK * (1 - fractionOfCurrentWeekElapsed));
  total += dailyRate * remainingCurrentWeek;
  remainingDays -= remainingCurrentWeek;

  while (remainingDays > 0) {
    dailyRate *= weeklyDecayRatio;
    const span = Math.min(DAYS_PER_WEEK, remainingDays);
    total += dailyRate * span;
    remainingDays -= span;
  }
  return total;
}
...
export function nextGenesisHalfLevel(epochSeconds, currentWeek) {
  assertFiniteNonNegative(epochSeconds, 'epochSeconds');
  assertFiniteNonNegative(currentWeek, 'currentWeek');
  const week = (Math.floor(currentWeek / DECAY_WEEKS_PER_HALF) + 1) * DECAY_WEEKS_PER_HALF;
  const asOf = new Date((epochSeconds + week * DAYS_PER_WEEK * 86_400) * 1_000).toISOString();
  return {
    week,
    asOf,
    s1DailyRate: emissionAtWeek(5_500, week),
    combinedDailyRate: emissionAtWeek(5_875, week),
  };
}
