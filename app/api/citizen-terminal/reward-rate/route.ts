...
async function deriveRewardRateSnapshot() {
  const url = rpcUrl();
...
const cachedRewardRateSnapshot = unstable_cache(
  deriveRewardRateSnapshot,
  ['citizen-reward-rate-v2'],
  { revalidate: REWARD_RATE_REVALIDATE_SECONDS },
);
